...
const seed = useState("showcase-seed-v2", () =>
    ...
    props: [
      {
        prop: "items",
        value: "MenuGroup[]",
        description: "Grouped menu options.",
      },
      {
        prop: "content",
        value: '{ align: "end" }',
    ...
    props: [
      {
        prop: "class",
        value: '"h-4 w-40"',
        description: "Custom shape and size.",
      },
    ],
  },
  {
...
const categoryCount = computed(() =>
  categories.reduce<Record<CategoryKey, number>>(
    (acc, category) => {
      acc[category.key] = demos.filter(
...
<template>
  <UContainer class="py-8 space-y-6">
    ...
    <UCard>
      <template #header>
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 class="text-2xl font-bold">Nuxt UI Categorized Showcase</h1>
    ...
    <div class="grid gap-4 lg:grid-cols-2">
      ...
      <UCard v-for="demo in pagedDemos" :key="demo.id">
        <template #header>
          <div>
        ...
        <div class="space-y-4">
          ...
          <template v-else-if="demo.id === 'pagination'">
            <UPagination
              ...
              :items-per-page="20"
            />
            <p class="text-sm text-muted">Current page: {{ demoPageNav }}</p>
          </template>
          ...
          <template v-else-if="demo.id === 'textarea'">
            <UForm :state="{ noteText }">
              <UFormField label="Note">
                <UTextarea v-model="noteText" :rows="3" />
              </UFormField>
            </UForm>
          </template>
          ...
          <template v-else-if="demo.id === 'check-radio'">
            <div class="space-y-3">
              ...
              <UFormField label="Schedule">
                <URadioGroup v-model="selectedPeriod" :items="periodOptions" />
              </UFormField>
            </div>
          </template>
          ...
          <template v-else-if="demo.id === 'switch-slider'">
            <div class="space-y-4">
              <USwitch v-model="enabled" label="Enable automation" />
              <UFormField label="Threshold">
                <USlider v-model="rangeValue" :min="0" :max="100" />
              </UFormField>
            </div>
          </template>
          ...
          <template v-else-if="demo.id === 'card'">
            <UCard class="bg-elevated/40">
              <template #header>
                <h3 class="font-medium">Nested UCard</h3>
              </template>
              <p class="text-sm text-muted">
          ...
          <template v-else-if="demo.id === 'chip-badge'">
            <div class="flex flex-wrap items-center gap-3">
              <UBadge color="primary" variant="subtle"> Stable </UBadge>
              <UBadge color="warning" variant="outline"> Beta </UBadge>
              <UChip inset>
                <UAvatar icon="i-lucide-user" />
              </UChip>
            </div>
          ...
          <template v-else-if="demo.id === 'buttons'">
            <div class="flex flex-wrap items-center gap-3">
              <UButton icon="i-lucide-play"> Primary </UButton>
              <UButton
                color="neutral"
                variant="outline"
              ...
              <UButtonGroup>
                ...
                />
                <UButton color="neutral" variant="subtle" label="Center" />
                <UButton
                  color="neutral"
                  variant="subtle"
        ...
        <template #footer>
          <p class="text-xs font-medium mb-2">Props Example</p>
          <UTable :columns="propColumns" :data="demo.props" />
        </template>
      </UCard>
    </div>
    ...
    <UCard>
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <p class="text-sm text-muted">
          Category:
          {{ categories.find((c) => c.key === activeCategory)?.label }} | Total:
          {{ totalForCurrent }} | Showing: {{ pagedDemos.length }}
        </p>
        <UPagination
